...
### 方式一：OpenClaw 对话式极速安装 (推荐)
如果你使用的是 **OpenClaw (小龙虾)** 终端，无需手动下载文件。
只需在对话框中向大模型发送以下指令即可：

> **“帮我从github上安装这个Skill，仓库地址是：`https://github.com/yyuchenBaii/ai_location_scout`，装好后告诉我。”**

OpenClaw 会在后台自动克隆此配置包并放到对应的 `.agents/skills` 目录。安装完成后，直接在对话中触发选址话题，系统将自动加载该专家能力并生成可视化研报。
